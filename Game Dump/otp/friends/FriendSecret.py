...
class FriendSecretNeedsParentPassword(StateData.StateData):
    ...
    def enter(self):
        ...
        if self.dialog == None:
            ...
            self.dialog = DirectFrame(parent = aspect2dp, pos = (0.0, 0.10000000000000001, 0.20000000000000001), relief = None, image = getDefaultDialogGeom(), image_color = OTPGlobals.GlobalDialogColor, image_scale = (1.3999999999999999, 1.0, 1.25), image_pos = (0, 0, -0.10000000000000001), text = OTPLocalizer.FriendSecretNeedsPasswordWarning, text_wordwrap = 20, text_scale = 0.055, text_pos = textPos, textMayChange = 1)
            ...
            if base.cr.productName != 'Terra-DMC':
                ...
                DirectButton(self.dialog, image = optionsButtonImage, image_scale = (2.125, 1.0, 1.0), relief = None, text = OTPLocalizer.ChangeSecretFriendsOptions, text_scale = 0.059999999999999998, text_pos = (0, -0.02), textMayChange = 0, pos = (0.0, 0.0, -0.59999999999999998), command = self._FriendSecretNeedsParentPassword__handleChangeOptions)
    ...
    def _FriendSecretNeedsParentPassword__handleChangeOptions(self):
        self.exit()
        base.localAvatar.chatMgr.fsm.request('noSecretChatWarning', [
            1])
...
class FriendSecret(DirectFrame, StateData.StateData):
    ...
    def __init__(self):
        DirectFrame.__init__(self, parent = aspect2dp, pos = (0, 0, 0.29999999999999999), relief = None, image = getDefaultDialogGeom(), image_scale = (1.6000000000000001, 1, 1.3999999999999999), image_pos = (0, 0, -0.050000000000000003), image_color = OTPGlobals.GlobalDialogColor, borderWidth = (0.01, 0.01))
    ...
    def load(self):
        ...
        self.introText.hide()
        guiButton = loader.loadModelOnce('phase_3/models/gui/quit_button')
        self.getSecret = DirectButton(parent = self, relief = None, pos = (0, 0, -0.11), image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (1, 1, 1), text = OTPLocalizer.FriendSecretGetSecret, text_scale = 0.059999999999999998, text_pos = (0, -0.02), command = self._FriendSecret__getSecret)
        self.getSecret.hide()
        self.enterSecretText = DirectLabel(parent = self, relief = None, pos = (0, 0, -0.27000000000000002), scale = 0.050000000000000003, text = OTPLocalizer.FriendSecretEnterSecret, text_fg = (0, 0, 0, 1), text_wordwrap = 30)
        ...
        self.ok1 = DirectButton(parent = self, relief = None, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (1, 1, 1), text = OTPLocalizer.FriendSecretEnter, text_scale = 0.059999999999999998, text_pos = (0, -0.02), pos = (0, 0, -0.5), command = self._FriendSecret__ok1)
        self.ok1.hide()
        if base.cr.productName == 'DisneyOnline-UK':
            ...
            self.changeOptions = DirectButton(self, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (2.125, 1.0, 1.0), relief = None, text = OTPLocalizer.ChangeSecretFriendsOptions, text_scale = 0.059999999999999998, text_pos = (0, -0.02), textMayChange = 0, pos = (0.0, 0.0, -0.65000000000000002), command = self._FriendSecret__handleChangeOptions)
        ...
        self.cancel.hide()
        self.nextText = DirectLabel(parent = self, relief = None, pos = (0, 0, 0.29999999999999999), scale = 0.059999999999999998, text = '', text_fg = (0, 0, 0, 1), text_wordwrap = 25.5)
        self.nextText.hide()
    ...
    def _FriendSecret__handleChangeOptions(self):
        self.exit()
        base.localAvatar.chatMgr.fsm.request('noSecretChatWarning', [
            1])

    
    def _FriendSecret__handleOkChangeOptions(self):
        base.localAvatar.chatMgr.fsm.request('activateChat')
