...
class OTPClientRepository(ClientRepository.ClientRepository):
    ...
    def __init__(self, serverVersion, launcher = None, playGame = None):
        ...
        self.loginFSM = ClassicFSM.ClassicFSM('loginFSM', [
            ...
            State.State('shutdown', self.enterShutdown, self.exitShutdown, [
                'loginOff']),
            State.State('waitForGameList', self.enterWaitForGameList, self.exitWaitForGameList, [
                'noConnection',
                'waitForShardList',
                'noShards']),
            ...
            State.State('waitForSetAvatarResponse', self.enterWaitForSetAvatarResponse, self.exitWaitForSetAvatarResponse, [
                ...
                'playingGame',
                'shutdown']),
            State.State('playingGame', self.enterPlayingGame, self.exitPlayingGame, [
                'noConnection',
                'waitForAvatarList',
                'login',
                'shutdown',
        ...
        if not wantOtpServer:
            self.gameFSM = ClassicFSM.ClassicFSM('gameFSM', [
            ...
            self.gameFSM = ClassicFSM.ClassicFSM('gameFSM', [
                ...
                State.State('playGame', self.enterPlayGame, self.exitPlayGame, [
                    'gameOff',
                    'closeShard']),
                State.State('closeShard', self.enterCloseShard, self.exitCloseShard, [
                    'gameOff'])], 'gameOff', 'gameOff')
    ...
    def enterWaitForGameList(self):
        self.handler = self.handleWaitForGetGameListResponse
        if not wantOtpServer:
            self.loginFSM.request('waitForShardList')
        else:
            self.setZonesRequested += 1
            self.rootInterestHandle = self.addInterest(OTP_DO_ID_SERVER_ROOT, OTP_ZONE_ID_MANAGEMENT, 'game directories')

    
    def handleWaitForGetGameListResponse(self, msgType, di):
    ...
    def enterWaitForShardList(self):
        ...
        if not wantOtpServer:
            ...
            self.setZonesRequested += 1
            self.shardInterestHandle = self.addInterest(self.gameRootDoId, OTP_ZONE_ID_DISTRICTS, 'LocalShardList')
    ...
    if not wantOtpServer:
        ...
        def handleWaitForGetShardListResponse(self, msgType, di):
            if msgType == CLIENT_DONE_SET_ZONE_RESP:
                ...
                numberOfShards = len(self.activeDistrictMap)
                if numberOfShards != 0:
    ...
    def handleDoneSetZoneResponseMsg(self, di):
        zoneId = di.getUint32()
    ...
    def detectLeakedTasks(self, extraTasks):
        allowedTasks = [
            'dataLoop',
            'resetPrevTransform',
            'doLaterProcessor',
    ...
    def detectLeakedEvents(self, extraHooks):
        allowedHooks = [
    ...
    def detectLeakedIntervals(self):
        if ivalMgr.getNumIntervals() > 0:
            print "You can't leave toontown until you clean up your intervals: {"
    ...
    def enterWaitOnEnterResponses(self, shardId, hoodId, zoneId, avId):
        ...
        self.handlerArgs = {
            ...
            'avId': avId }
        if wantOtpServer:
            print self.handlerArgs
            if not self.activeDistrictMap.has_key(shardId):
    ...
    def enterCloseShard(self):
        self.handler = self.handleCloseShard
        self.notify.info('Exiting shard ')
        self.sendSetShardMsg(0)
        self.waitForDatabaseTimeout(requestName = 'enterCloseShard')
    ...
    def enterPassThroughQuietZone(self, hoodId, zoneId, avId):
        ...
        self.handlerArgs = {
            ...
            'avId': avId }
        if wantOtpServer:
            print 'skyler quiet zone debug:', self.handlerArgs
    ...
    def handlePlayGame(self, msgType, di):
        ...
        if msgType == CLIENT_CREATE_OBJECT_REQUIRED:
            ...
            self.handleDoneSetZoneResponseMsg(di)
        elif msgType == CLIENT_GET_STATE_RESP:
            di.skipBytes(12)
            zoneId = di.getInt32()
        elif msgType == CLIENT_DONE_SET_ZONE_RESP:
        ...
        def disableAllBetweenShards(self):
            ...
            for distObj in distObjs:
                if distObj.doId == base.localAvatar.doId:
                    pass
                1
                if distObj.doId == OTP_DO_ID_FRIEND_MANAGER:
                    pass
                1
                self.disableDoId(distObj.doId)
    ...
    def sendSetZoneMsg(self, zoneId, visibleZoneList = None, parent = None):
        if wantOtpServer:
            if parent:
                base.localAvatar.setLocation(parent, zoneId)
            else:
                (oldParentId, oldZoneId) = base.localAvatar.getLocation()
        
        if wantOtpServer:
            ClientRepository.ClientRepository.sendSetZoneMsg(self, zoneId, visibleZoneList, parent)
        else:
            ClientRepository.ClientRepository.sendSetZoneMsg(self, zoneId, visibleZoneList)
        if not wantOtpServer:
            pass
        1
        self.setZonesRequested += 1
