...
class Movie(PandaObject.PandaObject):
    ...
    def playReward(self, ts, name, callback):
        ...
        (victory, camVictory) = MovieToonVictory.doToonVictory(self.battle.localToonActive(), self.battle.activeToons, self.toonRewardIds, self.toonRewardDicts, self.deathList, self.rewardPanel)
        if victory:
    ...
    def playTutorialReward(self, ts, name, callback):
        ...
        self.questList = self.rewardPanel.getQuestIntervalList(base.localAvatar, [
            0,
            1,
            1,
            0], [
    ...
    def playTutorialReward_1(self):
        self.tutRewardDialog_1 = TTDialog.TTDialog(text = TTLocalizer.MovieTutorialReward1, command = self.playTutorialReward_2, style = TTDialog.Acknowledge, fadeScreen = None, pos = (0.65000000000000002, 0, 0.5), scale = 0.80000000000000004)
        self.tutRewardDialog_1.hide()
        self.track = Sequence(name = 'tutorial-reward-1')
        self.track.append(Func(self.rewardPanel.initGagFrame, base.localAvatar, [
            0,
            0,
            0,
            0,
    ...
    def playTutorialReward_2(self, value):
        ...
        self.tutRewardDialog_2 = TTDialog.TTDialog(text = TTLocalizer.MovieTutorialReward2, command = self.playTutorialReward_3, style = TTDialog.Acknowledge, fadeScreen = None, pos = (0.65000000000000002, 0, 0.5), scale = 0.80000000000000004)
    ...
    def playTutorialReward_3(self, value):
        ...
        if self.questList:
            self.track1 = Sequence(Wait(1.0), Func(self.rewardPanel.initQuestFrame, base.localAvatar, copy.deepcopy(base.localAvatar.quests)), Wait(1.0), Sequence(*self.questList), Wait(1.0), Func(self.rewardPanel.hide), Func(camera.setPosHpr, render, 34, 19.879999999999999, 3.48, -90, -2.3599999999999999, 0), Func(base.localAvatar.animFSM.request, 'neutral'), Func(base.localAvatar.setPosHpr, 40.310000000000002, 22.0, -0.46999999999999997, 150.0, 360.0, 0.0), Wait(0.5), Func(self.tutorialTom.reparentTo, render), Func(self.tutorialTom.show), Func(self.tutorialTom.setPosHpr, 40.289999999999999, 17.899999999999999, -0.46999999999999997, 11.31, 0.0, 0.070000000000000007), Func(self.tutorialTom.animFSM.request, 'TeleportIn'), Wait(1.5169999999999999), Func(self.tutorialTom.animFSM.request, 'neutral'), Func(self.acceptOnce, self.tutorialTom.uniqueName('doneChatPage'), doneChat1), Func(self.tutorialTom.addActive), Func(music.setVolume, self.musicVolume), Func(self.tutorialTom.setLocalPageChat, TTLocalizer.MovieTutorialReward3, None, None, [
                self.tomDialogue03]), name = 'tutorial-reward-3a')
            self.track2 = Sequence(Func(self.acceptOnce, self.tutorialTom.uniqueName('doneChatPage'), doneChat2), Func(self.tutorialTom.setLocalPageChat, TTLocalizer.MovieTutorialReward4, 1, None, [
                self.tomDialogue04]), Func(self.tutorialTom.setPlayRate, 1.5, 'right-hand-start'), Func(self.tutorialTom.play, 'right-hand-start'), Wait(self.tutorialTom.getDuration('right-hand-start') / 1.5), Func(self.tutorialTom.loop, 'right-hand'), name = 'tutorial-reward-3b')
    ...
    def _Movie__doToonAttacks(self):
        if base.config.GetBool('want-toon-attack-anims', 1):
            ...
            (ival, camIval) = MovieNPCSOS.doNPCSOSs(self._Movie__findToonAttack(NPCSOS))
            if ival:
                track.append(ival)
                camTrack.append(camIval)
            
            (ival, camIval) = MoviePetSOS.doPetSOSs(self._Movie__findToonAttack(PETSOS))
            if ival:
                track.append(ival)
                camTrack.append(camIval)
    ...
    def genRewardDicts(self, id0, origExp0, earnedExp0, items0, missedItems0, origMerits0, merits0, parts0, id1, origExp1, earnedExp1, items1, missedItems1, origMerits1, merits1, parts1, id2, origExp2, earnedExp2, items2, missedItems2, origMerits2, merits2, parts2, id3, origExp3, earnedExp3, items3, missedItems3, origMerits3, merits3, parts3, deathList):
        ...
        entries = ((id0, origExp0, earnedExp0, items0, missedItems0, origMerits0, merits0, parts0), (id1, origExp1, earnedExp1, items1, missedItems1, origMerits1, merits1, parts1), (id2, origExp2, earnedExp2, items2, missedItems2, origMerits2, merits2, parts2), (id3, origExp3, earnedExp3, items3, missedItems3, origMerits3, merits3, parts3))
    ...
    def _Movie__findToonAttack(self, track):
        ...
        for ta in self.toonAttackDicts:
            if ta['track'] == track and track == NPCSOS and ta.has_key('special'):
                p.append(ta)
