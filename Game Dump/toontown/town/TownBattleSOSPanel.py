...
class TownBattleSOSPanel(DirectFrame, StateData.StateData):
    ...
    def makeFriendButton(self, friendPair):
        ...
        return DirectButton(relief = None, text = friendName, text_scale = 0.040000000000000001, text_align = TextNode.ALeft, text_fg = fg, text1_bg = self.textDownColor, text2_bg = self.textRolloverColor, text3_fg = self.textDisabledColor, command = com, extraArgs = [
            friendId])
    ...
    def _TownBattleSOSPanel__chosePet(self, friendId):
        doneStatus = { }
        doneStatus['mode'] = 'Pet'
        doneStatus['friend'] = friendId
        messenger.send(self.doneEvent, [
    ...
    def _TownBattleSOSPanel__updateScrollList(self):
        newFriends = []
        battlePets = base.config.GetBool('want-pets-in-battle', 0)
